...
export class ReviewsService {
  ...
  async submitReview(userId: string, interestId: string, dto: SubmitReviewDto) {
    ...
    if (eventDate && new Date(eventDate) > new Date()) {
      throw new BadRequestException(
        'You can only review a vendor after your event has taken place.',
      )
    }
    ...
    if (existing)
      throw new BadRequestException('You have already reviewed this vendor for this event.')
  ...
  async getReviewable(userId: string) {
    ...
    return (data ?? []).filter((i: any) => !(i.vendor_reviews?.length > 0) && i.events?.event_date)
  }
  ...
  private async recalculateVendorRating(
    ...
    const { data } = await client.from('vendor_reviews').select('rating').eq('vendor_id', vendorId)

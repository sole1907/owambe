...
const eventRow = { id: 'evt-1', title: 'Test Event', event_date: '2099-12-01', event_date_approximate: null, user_id: 'user-1', city: 'Lagos', guest_count_estimate: 100 }
const vendorRow = {
  id: 'ven-1',
  name: 'Vendor A',
...
const interestRow = { id: 'int-1', status: 'available', vendor_id: 'ven-1', event_id: 'evt-1', offered_price: 500000, counter_price: 600000, is_final_offer: false, events: eventRow, vendors: vendorRow }
...
describe('VendorInterestsService', () => {
  describe('getInterests()', () => {
    ...
    it('throws InternalServerErrorException on DB error for interests', async () => {
      ...
      const service = new VendorInterestsService(supabase as any, mockEmail as any as EmailService)
      await expect(service.getInterests('evt-1', 'user-1')).rejects.toThrow(InternalServerErrorException)
    })
  ...
  describe('addInterest()', () => {
    it('throws BadRequestException for invalid preference rank', async () => {
      const { service } = makeService()
      await expect(service.addInterest('evt-1', 'user-1', { vendorId: 'ven-1', preferenceRank: 4 } as any)).rejects.toThrow(BadRequestException)
      await expect(service.addInterest('evt-1', 'user-1', { vendorId: 'ven-1', preferenceRank: 0 } as any)).rejects.toThrow(BadRequestException)
    })
    ...
    it('throws NotFoundException when event not found', async () => {
      ...
      await expect(service.addInterest('evt-1', 'user-1', { vendorId: 'ven-1', preferenceRank: 1 } as any)).rejects.toThrow(NotFoundException)
    })
    ...
    it('throws BadRequestException when venue capacity exceeded', async () => {
      const supabase = makeSupabaseMock()
      supabase._mockFrom.mockImplementation((table: string) => {
        if (table === 'events') return q({ data: { ...eventRow, guest_count_estimate: 500 }, error: null })
        if (table === 'vendors') return q({ data: { ...vendorRow, capacity: 100, vendor_categories: { id: 'cat-1', name: 'Venues', slug: 'venues' } }, error: null })
        return q()
      })
      const service = new VendorInterestsService(supabase as any, mockEmail as any as EmailService)
      await expect(service.addInterest('evt-1', 'user-1', { vendorId: 'ven-1', preferenceRank: 1 } as any)).rejects.toThrow(BadRequestException)
    })
    ...
    it('throws BadRequestException when slot already taken', async () => {
      ...
      supabase._mockFrom.mockImplementation((table: string) => {
        ...
        if (table === 'vendor_interests') {
          ...
          if (callCount === 2) return q({ data: { id: 'int-slot', vendors: { name: 'Other Vendor' } }, error: null }) // slot taken
          return q()
        }
        return q()
      })
    ...
    it('inserts interest and sends email', async () => {
      ...
      const service = new VendorInterestsService(supabase as any, mockEmail as any as EmailService)
      const result = await service.addInterest('evt-1', 'user-1', { vendorId: 'ven-1', preferenceRank: 1, offeredPrice: 300000 } as any)
    ...
    it('sets status to unavailable when vendor has blocked event date', async () => {
      const blockedVendor = { ...vendorRow, vendor_availability: [{ date: '2099-12-01', status: 'blocked' }] }
      const supabase = makeSupabaseMock()
      let viCallCount = 0
      supabase._mockFrom.mockImplementation((table: string) => {
        if (table === 'events') return q({ data: eventRow, error: null })
      ...
      const service = new VendorInterestsService(supabase as any, mockEmail as any as EmailService)
      const result = await service.addInterest('evt-1', 'user-1', { vendorId: 'ven-1', preferenceRank: 1 } as any)
  ...
  describe('removeInterest()', () => {
    it('throws NotFoundException when interest not found', async () => {
      ...
      await expect(service.removeInterest('evt-1', 'int-1', 'user-1')).rejects.toThrow(NotFoundException)
    })
    ...
    it('throws InternalServerErrorException on delete error', async () => {
      ...
      const service = new VendorInterestsService(supabase as any, mockEmail as any as EmailService)
      await expect(service.removeInterest('evt-1', 'int-1', 'user-1')).rejects.toThrow(InternalServerErrorException)
    })
  ...
  describe('counterBack()', () => {
    it('throws NotFoundException when interest not found', async () => {
      ...
      await expect(service.counterBack('evt-1', 'int-1', 'user-1', 550000)).rejects.toThrow(NotFoundException)
    })
    ...
    it('throws BadRequestException when status is not quoted', async () => {
      ...
      await expect(service.counterBack('evt-1', 'int-1', 'user-1', 550000)).rejects.toThrow(BadRequestException)
    })

    it('throws BadRequestException when vendor counter is final', async () => {
      const { service } = makeService({
        vendor_interests: q({ data: { ...interestRow, status: 'quoted', is_final_offer: true }, error: null }),
      })
      await expect(service.counterBack('evt-1', 'int-1', 'user-1', 550000)).rejects.toThrow(BadRequestException)
    })
    ...
    it('updates interest and notifies vendor', async () => {
      ...
      supabase._mockFrom.mockImplementation((table: string) => {
        if (table === 'vendor_interests') {
          viCallCount++
          if (viCallCount === 1) return q({ data: { ...interestRow, status: 'quoted', is_final_offer: false }, error: null })
          return q({ data: { ...interestRow, status: 'pending', offered_price: 550000 }, error: null })
        }
        return q()
      })
      const service = new VendorInterestsService(supabase as any, mockEmail as any as EmailService)
  ...
  describe('acceptCounter()', () => {
    it('throws NotFoundException when interest not found', async () => {
      ...
      await expect(service.acceptCounter('evt-1', 'int-1', 'user-1')).rejects.toThrow(NotFoundException)
    })
    ...
    it('throws BadRequestException when status is not quoted', async () => {
      ...
      await expect(service.acceptCounter('evt-1', 'int-1', 'user-1')).rejects.toThrow(BadRequestException)
    })

    it('throws BadRequestException when counter_price is missing', async () => {
      const { service } = makeService({
        vendor_interests: q({ data: { ...interestRow, status: 'quoted', counter_price: null }, error: null }),
      })
      await expect(service.acceptCounter('evt-1', 'int-1', 'user-1')).rejects.toThrow(BadRequestException)
    })
    ...
    it('accepts counter and returns updated interest', async () => {
      ...
      supabase._mockFrom.mockImplementation((table: string) => {
        if (table === 'vendor_interests') {
          viCallCount++
          if (viCallCount === 1) return q({ data: { ...interestRow, status: 'quoted', counter_price: 600000 }, error: null })
          return q({ data: { ...interestRow, status: 'available', agreed_price: 600000 }, error: null })
        }
        return q()
      })
      const service = new VendorInterestsService(supabase as any, mockEmail as any as EmailService)
  ...
  describe('getInquiryCounts()', () => {
    ...
    it('returns pending count for found vendor', async () => {
      ...
      supabase._mockFrom.mockImplementation((table: string) => {
        ...
        if (table === 'vendor_interests') return q({ data: [{ status: 'pending' }, { status: 'pending' }], error: null })
        return q()
      })
      const service = new VendorInterestsService(supabase as any, mockEmail as any as EmailService)
      const result = await service.getInquiryCounts('user-1')
  ...
  describe('respondToInquiry()', () => {
    const respondVendorRow = { id: 'ven-1', name: 'Vendor A', email: 'vendor@test.com', commitment_fee_percentage: 30 }
    ...
    it('throws NotFoundException when vendor not found', async () => {
      ...
      await expect(service.respondToInquiry('user-1', 'int-1', { available: true } as any)).rejects.toThrow(NotFoundException)
    })
    ...
    it('throws BadRequestException when status is not pending/quoted', async () => {
      ...
      supabase._mockFrom.mockImplementation((table: string) => {
        if (table === 'vendors') return q({ data: respondVendorRow, error: null })
        if (table === 'vendor_interests') return q({ data: { ...respondInterestRow, status: 'committed' }, error: null })
        return q()
      })
      const service = new VendorInterestsService(supabase as any, mockEmail as any as EmailService)
      await expect(service.respondToInquiry('user-1', 'int-1', { available: true } as any)).rejects.toThrow(BadRequestException)
    })
    ...
    it('throws BadRequestException when offer is final but vendor tries to counter', async () => {
      ...
      supabase._mockFrom.mockImplementation((table: string) => {
        if (table === 'vendors') return q({ data: respondVendorRow, error: null })
        if (table === 'vendor_interests') return q({ data: { ...respondInterestRow, is_final_offer: true }, error: null })
        return q()
      })
      const service = new VendorInterestsService(supabase as any, mockEmail as any as EmailService)
      await expect(service.respondToInquiry('user-1', 'int-1', { available: true, counterPrice: 600000 } as any)).rejects.toThrow(BadRequestException)
    })
    ...
    it('marks as unavailable when vendor declines', async () => {
      ...
      expect(mockEmail.sendVendorResponse).toHaveBeenCalledWith(expect.objectContaining({ available: false }))
    })
    ...
    it('marks as quoted when vendor counters', async () => {
      ...
      supabase._mockFrom.mockImplementation((table: string) => {
        ...
        if (table === 'vendor_interests') {
          ...
          return q({ data: { ...respondInterestRow, status: 'quoted', counter_price: 600000 }, error: null })
        }
        return q()
      })
      const service = new VendorInterestsService(supabase as any, mockEmail as any as EmailService)
      const result = await service.respondToInquiry('user-1', 'int-1', { available: true, counterPrice: 600000 } as any)
    ...
    it('marks as available when vendor accepts offered price', async () => {
      ...
      supabase._mockFrom.mockImplementation((table: string) => {
        ...
        if (table === 'vendor_interests') {
          viCallCount++
          if (viCallCount === 1) return q({ data: respondInterestRow, error: null })
          return q({ data: { ...respondInterestRow, status: 'available', agreed_price: 500000 }, error: null })
        }
        return q()
      })
      const service = new VendorInterestsService(supabase as any, mockEmail as any as EmailService)
  ...
  describe('cancelBookingAsOrganiser()', () => {
    ...
    it('throws NotFoundException when booking not found', async () => {
      const { service } = makeService({ vendor_interests: q({ data: null, error: { message: 'not found' } }) })
      await expect(service.cancelBookingAsOrganiser('evt-1', 'int-1', 'user-1')).rejects.toThrow(NotFoundException)
    })

    it('throws BadRequestException when not authorised', async () => {
      const { service } = makeService({ vendor_interests: q({ data: { ...cancelledInterest, events: { ...cancelledInterest.events, user_id: 'other-user' } }, error: null }) })
      await expect(service.cancelBookingAsOrganiser('evt-1', 'int-1', 'user-1')).rejects.toThrow(BadRequestException)
    })

    it('throws BadRequestException when status is not cancellable', async () => {
      const { service } = makeService({ vendor_interests: q({ data: { ...cancelledInterest, status: 'pending' }, error: null }) })
      await expect(service.cancelBookingAsOrganiser('evt-1', 'int-1', 'user-1')).rejects.toThrow(BadRequestException)
    })
  ...
  describe('getCancellationStatus()', () => {
    it('throws NotFoundException when booking not found', async () => {
      const { service } = makeService({ vendor_interests: q({ data: null, error: { message: 'not found' } }) })
      await expect(service.getCancellationStatus('evt-1', 'int-1', 'user-1')).rejects.toThrow(NotFoundException)
    })

    it('throws BadRequestException when not authorised', async () => {
      const { service } = makeService({
        vendor_interests: q({ data: { id: 'int-1', events: { user_id: 'other-user' } }, error: null }),
      })
      await expect(service.getCancellationStatus('evt-1', 'int-1', 'user-1')).rejects.toThrow(BadRequestException)
    })

    it('throws NotFoundException when no cancellation found', async () => {
      const supabase = makeSupabaseMock()
      supabase._mockFrom.mockImplementation((table: string) => {
        if (table === 'vendor_interests') return q({ data: { id: 'int-1', events: { user_id: 'user-1' } }, error: null })
